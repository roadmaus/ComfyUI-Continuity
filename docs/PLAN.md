...
- [x] **7 — H3 stills (experimental).** `compile_still.py`,
  ...
  the sampled latent and decodes it through the family's own video VAE.
  ...
  frame 0 is a function of pixel frame 0 alone. The slice is handed to the VAE
  as two tokens rather than one — the shortest legal clip, the chosen token
  twice — because a single token is off the grid and decodes as tile seams and
  patch-grid noise (Comfy-Org/ComfyUI#15416). Keeping pixel frame 0 of the five
  that come back is exact for index 0, and it is what makes the branch need no
  image VAE of its own.
