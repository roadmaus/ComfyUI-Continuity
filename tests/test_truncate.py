"""The flow truncation's arithmetic, against a sampler it can be checked by hand on.

`truncate.py` claims two things: that a window of the whole schedule returns
exactly what the sampler returned, and that a narrower one returns the start
minus the windowed steps' velocities, weighted by their widths. Both are one
...
def by_hand(low, high, start_sigma=1.0, start=NOISE):
    """The paper's sum for the steps whose sigma falls in [low, high]."""
...
def run(low, high):
    trajectory = truncate.Trajectory(low, high)
...
trajectory, end = run(0.0, 1.0)
check("the whole schedule as the window is the sampler's own output",
...
trajectory, end = run(0.3, 0.7)
check("the paper's window is the start minus the windowed velocities' mean",
      close(trajectory.finish(end), by_hand(0.3, 0.7)), True)
check("...which is not what the sampler returned", close(trajectory.finish(end), end), False)
check("the window is inclusive at both ends: 0.3 to 0.7 takes five steps",
      round(trajectory.weight, 6), 0.5)

trajectory, end = run(0.3, 1.0)
check("the tail window keeps every step from 0.3 up",
      close(trajectory.finish(end), by_hand(0.3, 1.0)), True)

trajectory, end = run(0.91, 0.99)
check("a window no step lands in hands the sampler's output back untouched",
...
trajectory = truncate.Trajectory(0.3, 0.7)
...
acc = sum(VELOCITIES[i] * 0.1 for i in range(4, 10) if 0.3 <= float(SIGMAS[i]) <= 0.7)
check("a schedule entered at 0.6 is weighted over 0.6 from the latent it entered with",
      close(trajectory.finish(end), start - acc * (0.6 / 0.4)), True)
...
check("...its picture from the window", close(out.unbind()[0], by_hand(0.3, 0.7)), True)
...
trajectory = truncate.Trajectory(0.0, 1.0)
x = NOISE.clone()
...
base = FakePatcher()
patched = truncate._patch(base, 0.3, 0.7)
check("the patch clones rather than patching the model it was handed",
      (base.post_cfg, base.wrappers), ([], {}))
check("one sitting down the whole schedule is the windowed average",
      close(sitting(patched, SIGMAS, VELOCITIES, NOISE), by_hand(0.3, 0.7)), True)
...
opening_model = truncate._patch(FakePatcher(), 0.3, 0.7)
rest_model = truncate._patch(FakePatcher(), 0.3, 0.7)
...
check("the second sitting finishes the one sum from the first's start",
      close(finished, by_hand(0.3, 0.7)), True)
...
check("a sitting starting elsewhere is its own trajectory",
      close(own, NOISE - acc * (0.6 / 0.4)), True)
