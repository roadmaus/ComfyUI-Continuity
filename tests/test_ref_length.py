...
for length in (2.5, 6.6, 9.33, 12.0, 41.7):
    matched = canvas.match_seconds(length)
    ...
    check(f"{length}s matched compiles to its own count",
          compiled.frames, canvas.frames_for_seconds(length))
    # And it is the nearest count there is, which is the claim the pill makes.
    target = length * canvas.FPS
    nearest = min(canvas.legal_frame_counts(), key=lambda n: abs(n - target))
...
check("a long cue clamps", canvas.match_seconds(180),
      canvas.match_seconds(canvas.MAX_SECONDS))
check("a fragment clamps", canvas.match_seconds(0.2),
      canvas.match_seconds(canvas.MIN_SECONDS))
