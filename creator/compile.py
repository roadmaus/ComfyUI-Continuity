...
# wrote it. On, the tail is re-sampled from this far down the schedule against
# the shot's own references before the next pass conditions on it — see
# `Compiled.seam_restore`. The ceiling is the refine pass's, for the same
# reason: at 1.0 nothing of the frames being restored survives.
MIN_SEAM_RESTORE = 0.1
MAX_SEAM_RESTORE = 0.9
...
class Compiled:
    ...
    feather: int = 1
    # How far the inherited run is re-noised and re-sampled before this segment
    # conditions on it, or 0 for handed over as written (issue #41). Every
    # continued pass comes out a little softer than the one it continues, and
    # because the next seam anchors on that softer tail the loss compounds
    # down a strip. Restoring the tail first breaks the chain at every seam:
    # the run is re-drawn against the source shot's references and prompt, at
    # the same canvas, from partway down the schedule — enough to re-resolve
    # detail, not enough to move the picture. A family with no such pass
    # ignores it; see `base.Family.restores_seams`.
    seam_restore: float = 0.0
...
def seam_restore_denoise(raw):
    ...
    try:
        ...
        raise CompileError(f"seam_restore must be a number (got {raw!r})")
    if value <= 0:
        return 0.0
    return min(MAX_SEAM_RESTORE, max(MIN_SEAM_RESTORE, value))
...
def compile_request(data, image_size_lookup=None, continues=False, canvas_spec=None,
    ...
    # Only a seam has frames to restore; on a hard cut the value is a leftover,
    # read the same way a feather on a hard cut is.
    seam_restore = seam_restore_denoise(seam_restore) if continues else 0.0
...
def timeline_payloads(data, image_size_lookup=None):
    ...
    for position, (start, end) in enumerate(runs):
        ...
        if payload["continue"]:
            ...
            # The restore rides the seam the same way: validated where the
            # value is read, carried only when it is on.
...
def group_payload(data, start=0, end=None):
    ...
    for number, segment in enumerate(group, start=first_number):
        ...
        # neither of them is.
        if number == first_number and global_prompt and (not written
