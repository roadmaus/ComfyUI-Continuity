...
class ControlError(bench.BenchError):
...
def catalogue():
    ...
    """
    return {"tracings": bench.catalogue(TRACINGS)}


def _params(op, raw):
    """Whatever arrived -> the values this tracing actually takes.

    The clamping is `bench.values`; what is left here is the part that is this
    bench's own — an unknown tracing is a typo the caller has to see, where an
    out-of-range slider is not.
    """
    tracing = BY_ID.get(str(op))
    if tracing is None:
        raise ControlError(f"{op!r} is not a tracing this pack knows")
    return bench.values(tracing, raw, ControlError)
...
def _depth_model(name):
    def load():
        ...
        return comfy.sd.load_diffusion_model(bench.model_path(("geometry_estimation",), name))

    return bench.hold("depth", name, load)
...
def _depth(frame, values):
    ...
    with bench.ONE_AT_A_TIME:
        depth, _confidence, sky = _run_da3(
            model, bench.torch_frame(frame), int(values["detail"]))
    depth = depth[0]
    ...
    if values["invert"]:
        grey = 1.0 - grey
    return bench.from_torch(grey.unsqueeze(0).unsqueeze(-1).expand(1, *grey.shape, 3))
...
def _pose_models(model_name, vae_name):
    ...
    def load():
        ...
        model = comfy.sd.load_diffusion_model(
            bench.model_path(("checkpoints", "diffusion_models", "unet"), model_name))
    ...
    return bench.hold("pose", (model_name, vae_name), load)
    ...
    *those* colours: the eighteen limbs each have their own hue and the model
    ...
    with bench.ONE_AT_A_TIME:
        keypoints = SDPoseKeypointExtractor.execute(
            model, vae, bench.torch_frame(frame), 1).result[0]
    ...
    return bench.from_torch(drawn.float())
...
def _matte_model(name):
    ...
    def load():
        ...
        loaded = comfy.sd.load_checkpoint_guess_config(
            bench.model_path(("checkpoints",), name),
    ...
    return bench.hold("matte", name, load)
...
def _matte(frame, values):
    ...
    with bench.ONE_AT_A_TIME, _node_context():
        mask = node.execute(model, bench.torch_frame(frame),
...
# ---- the two answers ---------------------------------------------------------
#
# A frame for the light box, and a file for the input folder. Everything either
# one has to do to a source that is not the tracing itself — reading it, naming
# what comes out, carrying a clip's sound past the encoder — is `bench.py`.


def preview(path, op, raw, at=0.0):
    """-> (PNG bytes, `(width, height)`) of one frame, traced."""
    values = _params(op, raw)
    return bench.png(trace(bench.source_frame(path, at), op, values))
...
def _run_frame(path, op, values, out_dir, stem, at):
    ...
    render.
    """
    return bench.write_still(trace(bench.video_frame(path, at), op, values), out_dir, stem)
...
def run(filename, op, raw, trim=None, keep_sound=False, on_progress=None, at=None):
    ...
    if bench.is_video(path) and at is not None:
        at = max(0.0, float(at))
        name = _run_frame(path, op, values, out_dir, bench.stem(path, op, None, at), at)
        kind = "image"
    elif bench.is_video(path):
        # One frame at a time: a tracing is a function of the frame it is drawn
        # from and of nothing else, which is what lets the bench chase a running
        # clip in the first place.
        name = bench.transcode(path, out_dir, bench.stem(path, op, trim),
                               lambda frames: [trace(frame, op, values) for frame in frames],
                               trim=trim, keep_sound=keep_sound,
                               on_progress=on_progress, every=every)
        kind = "video"
    else:
        name = bench.write_still(trace(bench.open_image(path), op, values),
