...
# One piece of model work at a time. A heavy operator loads weights onto the GPU
# and the caller is a request handler on a thread pool, so without this a slider
# dragged twice would have two frames racing for the same weights — and the
# prompt queue is using that GPU too. One lock across every bench rather than one
# apiece for the same reason: there is one GPU, and the tracing bench and the
# upscale bench can both be open.
