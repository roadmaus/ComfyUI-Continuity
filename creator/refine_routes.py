...
def _picture(asset):
    ...
    # Nothing on disk to show: the timeline's storyboard is written at render
...
def _sighted(slot, asset, picture):
    ...
    if picture is not None:
        ...
        slot["picture"] = True
    elif compiler.is_storyboard(asset):
        slot["note"] = ("you cannot see it — it is made when the earlier shots have "
                        "rendered. Scope it as a scene reference: the place, its "
                        "surfaces and its light are retained, its framing and "
                        "camera are not")
