...
def _widgets():
    ...
    return [
        ...
        m.widget("sparse", "toggle", label="sparse attention", group="accel",
                 default=d["sparse"],
                 help="Skips most video tokens' keys and values inside each attention call. Composes with the attention backend underneath it."),
        m.widget("sparse_budget", "slider", label="sparse budget", group="accel",
                 default=d["sparse_budget"], min=0.01, max=1.0, step=0.01,
                 help="Fraction of video tokens kept dense. Lower is faster and cheaper; too low loses detail."),
        m.widget("sparse_backend", "combo", label="sparse backend", group="accel",
                 default=d["sparse_backend"], options=accel.SPARSE_BACKENDS),
        m.widget("sparse_schedule", "combo", label="sparse schedule", group="accel",
                 default=d["sparse_schedule"], options=accel.SPARSE_SCHEDULES),
        m.widget("sparse_token_order", "combo", label="sparse token order",
                 group="accel", default=d["sparse_token_order"],
                 options=accel.SPARSE_TOKEN_ORDERS),
        m.widget("memory", "toggle", label="memory optimization", group="accel",
                 default=d["memory"],
                 help="Chunks QKV and bounds MLP execution to lower peak VRAM. Composes with every other accelerator. Needs H3-Optimizations."),
        m.widget("memory_precision_mode", "combo", label="precision mode",
                 group="accel", default=d["memory_precision_mode"],
                 options=accel.MEMORY_PRECISION_MODES),
        m.widget("memory_qkv_streaming", "combo", label="QKV streaming",
                 group="accel", default=d["memory_qkv_streaming"],
                 options=accel.MEMORY_QKV_STREAMING_MODES),
        m.widget("memory_attention_memory", "combo", label="attention memory",
                 group="accel", default=d["memory_attention_memory"],
                 options=accel.MEMORY_ATTENTION_MEMORY_MODES),
    ]
