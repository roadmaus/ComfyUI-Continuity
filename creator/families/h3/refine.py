...
def slot_row(asset, label=None, show_label=False):
    ...
    # A file's row names the file. The timeline's storyboard has none yet —
    # it is made when the earlier shots have rendered — and is named by what
    # it is: their frames, as a video reference for the place in them.
    from ... import compile as compiler   # lazy: compile imports this package

    if compiler.is_storyboard(asset):
        what = ("the storyboard the timeline makes of the shots before this one — "
                "nine of their frames in time order, as a video reference for the "
                "place in them")
        row = {"handle": asset.handle, "what": what}
    else:
